...
function getLegacyRangeLabel(range = "7d") {
...
function getLegacyMaxDailyValue(items = []) {
...
export default function AdminAnalyticsPage() {
  ...
  return (
    <EditorShell
      ...
      eyebrow={null}
      description={null}
      description="Acompanhe visitas, cliques e origens da sua página pública."
      page={page}
      description={null}
      publishedPage={page}
      error={error}
        ...
        <div className="analytics-header__actions">
          <label className="analytics-range-select">
            ...
            <select
              ...
              aria-label="Selecionar período dos analytics"
            >
    ...
    >
      {loading ? (
        ...
        <div className="analytics-dashboard">
          ...
          <section className="analytics-overview-grid analytics-legacy-block">
            <article className="analytics-kpi-card">
              <span className="analytics-kpi-card__label">Visitas à página</span>
              <strong>{formatCardValue(analytics?.summary?.pageViews)}</strong>
              <small>{getRangeLabel(range)}</small>
            </article>

            <article className="analytics-kpi-card">
              <span className="analytics-kpi-card__label">Cliques em links</span>
              <strong>{formatCardValue(analytics?.summary?.linkClicks)}</strong>
              <small>{getRangeLabel(range)}</small>
            </article>

            <article className="analytics-kpi-card">
              <span className="analytics-kpi-card__label">Cliques em produtos</span>
              <strong>{formatCardValue(analytics?.summary?.productClicks)}</strong>
              <small>{getRangeLabel(range)}</small>
            </article>

            <article className="analytics-kpi-card">
              <span className="analytics-kpi-card__label">Origem principal</span>
              <strong>{analytics?.summary?.primaryOrigin || getTopOriginLabel(analytics?.origins)}</strong>
              <small>{getRangeLabel(range)}</small>
            </article>
          </section>
          ...
          <div className="analytics-sections-grid analytics-legacy-block">
            <SectionCard
              title="Origens"
            ...
            <SectionCard
              title="Série diária"
              description="Resumo diário de visitas e cliques no período selecionado."
            >
              {analytics?.daily?.length ? (
                <div className="analytics-daily-list">
                  {analytics.daily.map((item) => {
                    ...
                    const width = maxDailyValue ? `${Math.max(8, (highestValue / maxDailyValue) * 100)}%` : "8%";

                    return (
                      <div key={item.date} className="analytics-daily-list__item">
                        <div className="analytics-daily-list__copy">
                          <strong>{item.date}</strong>
                          <span>
                            {formatCardValue(item.pageViews)} visitas · {formatCardValue(item.linkClicks)} cliques em links ·{" "}
                            {formatCardValue(item.productClicks)} cliques em produtos
                          </span>
                        </div>
                        <div className="analytics-daily-list__bar">
                          <span style={{ width }} />
                        </div>
                      </div>
                    );
                ...
                <div className="empty-state">Ainda não há eventos suficientes para montar a série diária.</div>
              )}
            </SectionCard>
          </div>
