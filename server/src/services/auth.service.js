...
async function ensureUniqueSlug(baseSlug = "") {
  const safeBaseSlug = normalizeSlug(baseSlug) || "minha-página";
...
function buildInitialPage(ownerId, { email = "", displayName = "" } = {}) {
  ...
  const fallbackTitle = normalizedDisplayName || getEmailLocalPart(email) || "Minha página";

  return {
    ownerId,
    title: fallbackTitle,
    slug: buildInitialSlug(email, normalizedDisplayName),
    bio: "Compartilhe tudo o que importa em um só lugar.",
    avatarUrl: "",
    links: [],
    secondaryLinks: [],
    shop: {
...
export async function registerUser(payload = {}) {
  ...
  if (!email || !/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(email)) {
    throw createHttpError(400, "Informe um e-mail válido.", "AUTH_INVALID_EMAIL");
  }

  if (await User.exists({ email })) {
    throw createHttpError(409, "Este e-mail já está em uso.", "AUTH_EMAIL_ALREADY_EXISTS");
  }
...
export async function loginUser(payload = {}) {
  ...
  if (!user || !(await verifyPassword(password, user.passwordHash))) {
    throw createHttpError(401, "E-mail ou senha inválidos.", "AUTH_INVALID_CREDENTIALS");
  }
...
export async function getAuthUserById(userId = "") {
  ...
  if (!user) {
    throw createHttpError(401, "Sessão inválida ou expirada.", "AUTH_USER_NOT_FOUND");
  }
